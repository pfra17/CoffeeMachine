...
{   //Deklaration der Strukturfelder 1-4 (bzw 0-3),der summe,index,posten,abschluss,anzahl
    //Aufruf von der Logo Funktion f�r die Anzeige
    //Vorbereitungen zur �bergabe an ein TXT_File
    FILE *fp;
    ...
    //Begr�ssung und Anzeige, do while Schleife: Falls abgebrochen wird, startet Anzeige neu,
    //If Bedingung 1: Aufnahme der Betsellung und �bergabe an die Funktion2,
    ...
    while (schleife==1);
        ...
        if (schleife==0)
        {
        //Abfrage ob Bestellung ausgef�hrt werden soll,
        //1: Bedanken und Rechnung drucken,�bergabe der Werte an Funktion1 zur Einzelsummenberechnung f�r die Rechnung und Abfrage der Funktion2 f�r die Gesamtsumme aller Bestellungen.
        ...
        if(abschluss==1)
            {
                ...
                fprintf(fp,"###Vielen Dank f�r ihren Einkauf###\n");
    ...
    {
                 for (delay_a=1; delay_a<= 4; delay_a++){                   //Simulation des Geldz�hlers
                        printf(".");
                ...
                printf("Ihre Bestellung wird zubereitet.\n");
                        for (delay_a=1; delay_a<= 10; delay_a++){                   //Delay f�r Kaffeezubereitung
                            printf(".");
                            for (delay_b = 1; delay_b <= 32767; delay_b++){
                                for (delay_c = 1; delay_c <= 32767; delay_c++){}
                }}
